...
struct Config {
    ...
    static let colors = [0xFF3300, 0x33CC33, 0x2558D5, 0xECA23F, 0xF47153].map { val in Color(hex: val) }
...
struct SliderData {
    ...
    let centerY: Double
    let progress: Double
    let finishingFunc: () -> Void

    init(side: SliderSide) {
        self.init(side: side, centerY: side == .left ? Config.leftWaveY : Config.rightWaveY, progress: 0,finishingFunc: {})
    }
    func setFinishingFunc(finishingFunc : @escaping () -> Void) -> SliderData {
        return SliderData(side: side, centerY: centerY, progress: progress,finishingFunc: finishingFunc)
    }

    init(side: SliderSide, centerY: Double, progress: Double,finishingFunc : @escaping () -> Void) {
        self.side = side
        self.centerY = centerY
        self.progress = progress
        self.finishingFunc = finishingFunc;
    }
    ...
    func initial() -> SliderData {
        return SliderData(side: side, centerY: centerY, progress: 0,finishingFunc: self.finishingFunc)
    }
    ...
    func final() -> SliderData {
        ...
        return SliderData(side: side, centerY: centerY, progress: 1,finishingFunc: self.finishingFunc)
    }
    ...
    func drag(value: DragGesture.Value) -> SliderData {
        ...
        return SliderData(side: side, centerY: Double(value.location.y), progress: progress,finishingFunc: self.finishingFunc)
    }
